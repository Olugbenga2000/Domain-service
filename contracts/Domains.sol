...
contract Domains is ERC721URIStorage{
  ...
  error InvalidName(string name);
    ...
    constructor(string memory _tId) ERC721('Favour Name Service', 'FNS'){
        tId = _tId;
        console.log("%s domain service deployed", _tId);
        owner = msg.sender;
    }
  ...
  require(msg.value == _price, "Wrong amount of matic");
	
  string memory _name = string(abi.encodePacked(name, ".", tId));
    ...
    bytes(
      string(
        abi.encodePacked(
          '{"name": "',
          _name,
          '", "description": "A domain on the Favour name service", "image": "data:image/svg+xml;base64,',
